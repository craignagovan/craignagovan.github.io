...
class modInstallerPts {
    ...
    static public function check($extPlugName = '') {
		...
		if($modules = self::_getExtendModules($locations)) {
			foreach($modules as $m) {
				if(!empty($m)) {
					//If module Exists - just activate it, we can't check this using framePts::moduleExists because this will not work for multy-site WP
					if(framePts::_()->getTable('modules')->exists($m['code'], 'code') /*framePts::_()->moduleExists($m['code'])*/) {
						self::activate($m);
    ...
    static public function deactivate() {
		...
		if($modules = self::_getExtendModules($locations)) {
			foreach($modules as $m) {
				if(framePts::_()->moduleActive($m['code'])) { //If module is active - then deacivate it
    ...
    static public function activate($modDataArr) {
		if(!empty($modDataArr['code']) && !framePts::_()->moduleActive($modDataArr['code'])) { //If module is not active - then acivate it
			if(framePts::_()->getModule('options')->getModel('modules')->put(array(
    ...
    static public function uninstall() {
		...
		if($modules = self::_getExtendModules($locations)) {
			foreach($modules as $m) {
				...
				framePts::_()->getModule('options')->getModel('modules')->delete(array('code' => $m['code']));
